...
export async function readBoundedLines(options: BoundedLineReaderOptions): Promise<void> {
  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      while (offset < bytes.length) {
        ...
        if (newline === -1) {
          ...
          if (unfinished.length + fragment.length > MAX_SESSION_ARTIFACT_LINE_BYTES + 1) throw new Error(options.errorMessage);
    ...
    if (unfinished.length > MAX_SESSION_ARTIFACT_LINE_BYTES) throw new Error(options.errorMessage);
    if (unfinished.length > 0 || !options.skipEmptyLines) await options.onLine(unfinished.toString('utf8'));
  } catch {
