...
- [ ] **Step 1: Write failing ingress and CLI tests**

Add a non-Git temporary current directory to Cursor hook and both CLI test fixtures. Assert a rejected Cursor event increments only its workspace scope, automatic current-directory selection uses that scope, explicit non-Git directory selection returns the same scope, and a Git nested directory still resolves to its Git top level. Assert JSON and text output omit the temporary path and show the scope kind and derived ID; assert symlinked workspace selections resolve to one scope.
...
- [ ] **Step 3: Implement boundary integration**

Replace repository-only scope resolution in hook ingress and the application service with `resolveDiagnosticScope`. Keep existing primary capture repository behavior unchanged. Both CLI forms call the one application-service report method and format the same `scope` object. Catch resolver and diagnostic-store errors at hook ingress, preserving its existing empty stdout, generic stderr and zero exit status.
...
- [ ] **Step 1: Write failing workspace acceptance coverage**
