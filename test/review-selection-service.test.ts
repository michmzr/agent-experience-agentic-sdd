...
test('runs only the confirmed latest artifact from an injected verified repository-scoped discovery', async () => {
  ...
  const review = await runManualReview(
    { source: 'codex', root, session: 'latest', interactive: true, repository: 'repo-current', allowExpensiveChecks: false },
    {
      discover: async () => [{ source: 'codex', id: 'selected.jsonl', location: join(root, 'selected.jsonl'), repositoryHint: 'repo-current', repositoryHintVerified: true, updatedAt: '2026-08-24T10:00:00.000Z' }],
