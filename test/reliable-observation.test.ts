...
function parseFixture(value: unknown): Fixture {
  ...
  assertClosedKeys(value, ['version', 'synthetic', 'scenarios', 'receipts', 'results', 'analysis', 'findings', 'abstentions', 'expectedQuality'], 'fixture');
  assert.equal(value.version, 1, 'fixture.version');
  assert.equal(value.synthetic, true, 'fixture.synthetic');
  for (const key of ['scenarios', 'receipts', 'results', 'analysis', 'findings', 'abstentions'] as const) assertArray(value[key], `fixture.${key}`);
  ...
  for (const scenario of value.scenarios as unknown[]) {
    ...
    assertClosedKeys(scenario.expectedRelationships, ['receiptIds', 'resultIds'], 'scenario.expectedRelationships');
  ...
  const receiptIds = validateReceipts(value.receipts as unknown[], scenarioIds);
  const resultIds = validateResults(value.results as unknown[], scenarioIds);
  validateAnalysis(value.analysis as unknown[], scenarioIds);
  validateReferences(expectedReceiptIds, receiptIds, 'receipt');
  validateReferences(expectedResultIds, resultIds, 'result');
  validateFindings(value.findings as unknown[], scenarioIds, 'findings');
  validateAbstentions(value.abstentions as unknown[], scenarioIds);
...
function validateReceipts(records: unknown[], scenarioIds: ReadonlySet<string>): Map<string, string[]> {
...
function validateResults(records: unknown[], scenarioIds: ReadonlySet<string>): Map<string, string[]> {
  const ids = new Map<string, string[]>();
  for (const record of records) {
    ...
    if (record.unknownReason !== undefined) assertOneOf(record.unknownReason, ['result-not-delivered', 'privacy-redacted', 'verification-not-observed'], 'result.unknownReason');
...
function validateAbstentions(records: unknown[], scenarioIds: ReadonlySet<string>): void {
  for (const record of records) {
    ...
    for (const evidenceGap of record.evidenceGaps) assertOneOf(evidenceGap, ['result-not-delivered', 'privacy-redacted', 'verification-not-observed'], 'abstention.evidenceGaps');
  }
}

function validateReferences(expected: ReadonlyMap<string, readonly string[]>, actual: ReadonlyMap<string, readonly string[]>, name: string): void {
  for (const ids of expected.values()) for (const id of ids) assert.ok(actual.has(id), `expected ${name} relationship is missing: ${id}`);
}

function validateExpectedQuality(value: Record<string, unknown>): void {
  assertClosedKeys(value, ['uniqueOperations', 'durableReceipts', 'linkedResults', 'skips', 'unknownReasons', 'analysisStates', 'findings', 'abstentions', 'cost'], 'fixture.expectedQuality');
  for (const key of ['uniqueOperations', 'durableReceipts', 'linkedResults', 'findings', 'abstentions', 'cost']) {
