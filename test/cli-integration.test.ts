...
test('imports, validates, lists, inspects, retrieves, and exports a fixture', () => {
  ...
  try {
    ...
    assert.match(runCli(['retrieve', '--scope', 'repository', '--repository-id', 'repo-a', '--path', 'src/a.ts', '--tool', 'git', '--tag', 'safety', '--json', '--data-dir', dataDir]).stdout, /knowledge-1/);
    const exported = runCli(['export', '--scope', 'repository', '--repository-id', 'repo-a', '--json', '--data-dir', dataDir]);
...
test('keeps fixture scope, authority, lifecycle, and serialization deterministic', () => {
  ...
  try {
    ...
    const repositoryA = JSON.parse(runCli(['retrieve', '--scope', 'repository', '--repository-id', 'repo-a', '--json', '--data-dir', dataDir]).stdout);
